  ...
  (0, _createClass2.default)(Spec, [{
    ...
    value: function stop(err) {
      ...
      if (!err) {
        ...
        var message = 'Failed with no falsy reason.';
        this.reject(this.error = {
          message: message,
          trace: ''
        });
      } else {
        (0, _sourcemappedStacktrace.mapStackTrace)(err.stack, function (mapped) {
          var filter = function filter(line) {
            return !/node_modules/.test(line);
          };

          var coerce = function coerce(line) {
            return line.replace(/\(\.\.\//, '(');
          };

          var source = mapped.filter(filter).map(coerce);

          _this2.reject(_this2.error = (0, _objectSpread2.default)({}, err, {
            trace: source.join('\n')
          }));
        });
      }
